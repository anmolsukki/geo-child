...
class ProfilePage extends Component {
  ...
  submitChangePassword = () => {
    const data = {
      currentPassword: this.state.changePassword.currentPass,
      password: this.state.changePassword.newPass,
      passwordConfirm: this.state.changePassword.confirmPass,
    };
    this.props.changePasswordActionData(data);
  };
              ...
              <div className="sectionView">
                <ul className="sectionContainer">
                  <li className="sectionList" onClick={() => this.goToStep(1)}>
                    <Link
                      to="#"
                      className={`sectonName ${this.state.onStep === 1 ? 'active' : ''}`}>
                      My Magazine
                    </Link>
                  </li>
                  <li className="sectionList" onClick={() => this.goToStep(2)}>
                    <Link
                      to="#"
                      className={`sectonName ${this.state.onStep === 2 ? 'active' : ''}`}>
                      Profile
                    </Link>
                  </li>
                  <li className="sectionList" onClick={() => this.goToStep(3)}>
                    <Link
                      to="#"
                      className={`sectonName ${this.state.onStep === 3 ? 'active' : ''}`}>
                      Change Password
                    </Link>
                  </li>
                  <li className="sectionList" onClick={() => this.goToStep(4)}>
                    <Link
                      to="#"
                      className={`sectonName ${this.state.onStep === 4 ? 'active' : ''}`}>
                      Logout
                    </Link>
                  </li>
                </ul>
              ...
              <div className="bodyView formbody">
                {this.state.onStep === 1 ? (
                  ...
                  <div>
                    ...
                    <div className="rule"></div>
                    <div className="form-footer">
                      <Link to="#" onClick={this.submitChangePassword}>
                        Submit!<span className="fa fa-thumbs-o-up"></span>
                      </Link>
                    </div>
                  </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    changePasswordActionData: (data) => dispatch(actionCreator.changePasswordAction(data)),
  };
};
